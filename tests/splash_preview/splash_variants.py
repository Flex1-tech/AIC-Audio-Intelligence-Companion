...
Variantes :
  ...
  - SplashV3Immersive  (Variante B : Carte adaptative 130-170 px, equalizer 45px, 2.5s)
  - SplashV3Fullscreen (Variante C : Fullscreen maîtrisée sans carte, logo 160-240px, marges généreuses, 2.8s)
"""
...
class SplashV3Current(ft.Container):
    ...
    LABEL = "Variante A — V3 Baseline (110px)"
...
# ══════════════════════════════════════════════════════════════════════════════
# VARIANTE B : SplashV3Immersive (Carte adaptative 135-160px, amplitude 45px, 2.5s)
# ══════════════════════════════════════════════════════════════════════════════


class SplashV3Immersive(ft.Container):
    """Variante B — V3 Immersive (carte adaptative 135-160px, amplitude 45px, 2.5s)."""
    ...
    def __init__(self, page: ft.Page, on_complete: Optional[Callable[[], None]] = None):
        ...
        card_size = int(max(130, min(160, min(w, h) * 0.22)))
    ...
    async def start_animation_async(self) -> None:
        ...
        try:
            ...
            self.logo_box.shadow = ft.BoxShadow(spread_radius=9, blur_radius=38, color="#48FE8F40")
            self.bar1.height = 18
            self.bar2.height = 30
            self.bar3.height = 45
            self.bar4.height = 30
            ...
            self.logo_box.shadow = ft.BoxShadow(spread_radius=3, blur_radius=20, color="#20FE8F40")
            _safe_update(self.logo_box)
...
class SplashV3Fullscreen(ft.Container):
    """Variante C — V3 Fullscreen maîtrisée (logo adaptatif 180-240px, marges généreuses, 2.8s)."""

    LABEL = "Variante C — V3 Fullscreen Maîtrisée (200px open)"
    ...
    def __init__(self, page: ft.Page, on_complete: Optional[Callable[[], None]] = None):
        ...
        # 7 barres d'égaliseur ouvertes sous le logo
        ...
        # Halo ambre ambiant centré sur le logo ouvert
    ...
    async def start_animation_async(self) -> None:
        cb_called = False
        try:
            # 1. Éveil du fond ambiant & Logo (t=0 -> 100ms)
            ...
            # 2. Éveil de l'égaliseur (t=450ms)
            await asyncio.sleep(0.40)
            self.wave_container.opacity = 1.0
            _safe_update(self.wave_container)

            # 3. Impulsion des barres & halo ambiant (t=800ms)
            await asyncio.sleep(0.35)
            self.bar1.height = 20
            self.bar2.height = 36
            self.bar3.height = 54
            ...
            # 4. Titre "AIC" (t=1150ms)
            await asyncio.sleep(0.30)
            self.title_box.opacity = 1.0
            self.title_box.offset = ft.Offset(0, 0)
            _safe_update(self.title_box)

            # 5. Sous-titre Ambre (t=1450ms)
            await asyncio.sleep(0.25)
            self.subtitle_box.opacity = 1.0
            self.subtitle_box.offset = ft.Offset(0, 0)
            _safe_update(self.subtitle_box)

            # 6. Maintien contemplatif & fondu de sortie (t=2100ms -> 2800ms)
...
VARIANT_ORDER = ["v3current", "v3immersive", "v3fullscreen"]

VARIANT_LABELS = {
    "v3current": "A — V3 Current (110px)",
